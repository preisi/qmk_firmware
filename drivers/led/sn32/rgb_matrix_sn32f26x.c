...
static uint8_t        rgb_row_idx                     = 0;    // LED row scan counter
...
RGB                   rgb_matrix_sn32f26x[DRIVER_LED_TOTAL];  // led state buffer

void rgb_callback(PWMDriver *pwm_driver);
...
// map matrix column pin to respective pwm channel
    ...
    for (uint8_t i = 0; i < LED_MATRIX_COLS; i++) {
        switch (led_col_pins[i]) {
            ...
            case D3:
                chan_col_order[i] = 19;
                break;
            /* Channel 20 is a dummy channel in 26x .*/
            case D5:
                chan_col_order[i] = 21;
                break;
...
void advance_row(void) {
    ...
    rgb_row_idx = (rgb_row_idx + 1) % LED_MATRIX_ROWS_HW;

    // advance to the next key matrix row if necessary
    if (rgb_row_idx % LED_MATRIX_ROW_CHANNELS == 2) {
        row_idx = (row_idx + 1) % LED_MATRIX_ROWS;
    }
}

void reconfigure_rgb(PWMDriver *pwm_driver, uint8_t row_idx, uint8_t col_idx) {
    uint8_t led_idx = g_led_config.matrix_co[row_idx][col_idx];

    // check if we need to enable PWM output for the current RGB row channel
    // and update matching RGB channel PWM configuration if necessary
    uint8_t row_channel = rgb_row_idx % LED_MATRIX_ROW_CHANNELS;
    if (row_channel == 0 && rgb_matrix_sn32f26x[led_idx].b != 0) {
        pwmEnableChannelI(pwm_driver, chan_col_order[col_idx], rgb_matrix_sn32f26x[led_idx].b);
    } else if (row_channel == 1 && rgb_matrix_sn32f26x[led_idx].g != 0) {
        pwmEnableChannelI(pwm_driver, chan_col_order[col_idx], rgb_matrix_sn32f26x[led_idx].g);
    } else if (row_channel == 2 && rgb_matrix_sn32f26x[led_idx].r != 0) {
        pwmEnableChannelI(pwm_driver, chan_col_order[col_idx], rgb_matrix_sn32f26x[led_idx].r);
    }
}

void scan_matrix_and_reconfigure_rgb(PWMDriver *pwm_driver) {
    // disable previously configured RGB channel pin
    writePinLow(led_row_pins[rgb_row_idx]);
    // disable pwm outputs on all column pins
    for (uint8_t col_idx = 0; col_idx < LED_MATRIX_COLS; col_idx++) {
        pwmDisableChannel(&PWMD1, chan_col_order[col_idx]);
    }
    advance_row();
...
#endif
        reconfigure_rgb(pwm_driver, row_idx, col_idx);
    }
    // enable newly configured RGB channel pin
    writePinHigh(led_row_pins[rgb_row_idx]);
}
...
void rgb_callback(PWMDriver *pwm_driver) {
    ...
    chSysLockFromISR();
    scan_matrix_and_reconfigure_rgb(pwm_driver);
    chSysUnlockFromISR();

    // Advance the timer to just before the wrap-around, that will start a new PWM cycle
    pwm_lld_change_counter(pwm_driver, 0xFFFF);
...
void SN32F26x_init(void) {
    ...
    // Determine which PWM channels we need to control each rgb column
    rgb_ch_ctrl();
...
void SN32F26x_set_color(int index, uint8_t r, uint8_t g, uint8_t b) {
    rgb_matrix_sn32f26x[index].r = r;
    rgb_matrix_sn32f26x[index].b = b;
    rgb_matrix_sn32f26x[index].g = g;
}

void SN32F26x_set_color_all(uint8_t r, uint8_t g, uint8_t b) {
    for (int i = 0; i < DRIVER_LED_TOTAL; i++) {
        SN32F26x_set_color(i, r, g, b);
    }
}
